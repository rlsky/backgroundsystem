  ...
  <div>
    ...
    <el-popover placement="right" trigger="click" v-model="visible">
      <el-transfer :titles="['隐藏列', '展示列']" :button-texts="['隐藏', '展示']"  v-model="value" :data="data" @change="transferChange">
        <el-button class="transfer-footer left-transfer" slot="left-footer" size="small"></el-button>
        <el-button class="transfer-footer" slot="right-footer" size="small" @click="toSave('1')">保存更改</el-button>
      </el-transfer>
    ...
    <el-table :data="tableData" stripe border style="width: 98%" ref="tableDataRef">
          ...
          <template v-for="(item, index) in data">
            <el-table-column v-if="item.isShow" :prop="item.prop" :label="item.label" align="center"></el-table-column>
          </template>
    </el-table>
  </div>
</template>
...
export default {
  ...
  methods: {
    // 获取初始展示列
    getValue() {
      let valueArr=JSON.parse(localStorage.getItem('showvalue'));
      //如果有缓存，把缓存的值赋值给value
    ...
    toSave(flag) {
      if(flag !== '1'){
        ...
        // 缓存记忆
        // 1. 获取缓存
        let getArr=JSON.parse(localStorage.getItem('showvalue'))

  ...
  <el-container>
    ...
    <el-container>
      ...
      <el-aside :width="isCollapsed? '64px' : '200px'">
        <div :style="{width:isCollapsed? '64px' : '200px' }" class="asideSty">
          <div class="iconWrap" @click="toqiehuan">
            <i :class="[isCollapsed?'el-icon-s-unfold':'el-icon-s-fold']" class="zhedie"/>
          </div>
          <el-menu
      ...
      <el-main class="main">
        <div class="content">
          <!-- 面包屑：首页不展示 -->
          <Crumbs v-if="$route.path != '/home'" :bread-list="breadList"/>
          <router-view/>
        </div>
      </el-main>
    </el-container>
...
export default {
  ...
  methods: {
    ...
    checkAuth() {
      ...
      const token = this.$store.state.login.token || (sessionInfo && sessionInfo.login.token)
      if (!token) {
